"""
Training script for Dataset 2 PatchTST Benchmark Model.
"""
...
def train_patchtst(
    ...
    """
    Train PatchTST Benchmark Model on Dataset 2 eye-tracking sequences.
    """
    set_seed(config.get("random_seed", 42))

    device = get_device(config.get("device", "auto"))
    print(f"[PatchTST Training] Using compute device: {device}")
    ...
    lr = patch_cfg.get("learning_rate", 5e-4)
    weight_decay = patch_cfg.get("weight_decay", 1e-4)
    epochs = patch_cfg.get("epochs", 25)
    batch_size = patch_cfg.get("batch_size", 64)
    patience = patch_cfg.get("early_stopping_patience", 8)
    ...
    print(
        f"[PatchTST Architecture] Layers={n_layers}, d_model={d_model}, heads={n_heads}, "
        f"patch_len={patch_len}, stride={stride}, patches={model.num_patches}, dropout={dropout}"
    )
    ...
    best_checkpoint_path = os.path.join(checkpoint_dir, "patchtst_best.pt")
    ...
    print(f"[PatchTST Training] Starting {epochs} epochs (batch_size={batch_size}, lr={lr})...")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            print(f"  --> Saved new best PatchTST checkpoint (Val Loss: {best_val_loss:.4f})")
        else:
            patience_counter += 1
            if patience_counter >= patience:
                print(f"[PatchTST Early Stopping] Triggered after {epoch} epochs (patience={patience}).")
                break

    total_training_time = time.time() - start_time
    print(f"[PatchTST Training] Completed in {total_training_time:.2f} seconds.")

    # ---- Final Evaluation on Test Set using Best Model ----
    print(f"\n[PatchTST Evaluation] Loading best model from {best_checkpoint_path} for testing...")
    ...
    plt.title("PatchTST — Loss Curves", fontsize=12, fontweight="bold")
    plt.xlabel("Epoch")
    plt.ylabel("BCE Loss")
    plt.legend()
    ...
    plt.title("PatchTST — Validation Accuracy", fontsize=12, fontweight="bold")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    curve_path = os.path.join(plots_dir, "patchtst_training_curves.png")
    plt.savefig(curve_path, dpi=150)
    plt.close()
    print(f"Saved PatchTST training curve to {curve_path}")

    # 2. Confusion Matrix Plot
    cm_path = os.path.join(cm_dir, "patchtst_confusion_matrix.png")
    plot_confusion_matrix(
        np.array(test_metrics["confusion_matrix"]),
        class_names=["TD", "ASD"],
        title="PatchTST — Confusion Matrix (Dataset 2 Test Set)",
        save_path=cm_path,
    )
    print(f"Saved PatchTST confusion matrix to {cm_path}")

    # 3. Save JSON Metrics
    metrics_path = os.path.join(metrics_dir, "patchtst_metrics.json")
    save_payload = {
        "model": "PatchTST",
    ...
    print(f"Saved PatchTST metrics to {metrics_path}")
